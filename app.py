...
st.set_page_config(
    page_title="Sidewalk AI Scoring Tool",
    layout="wide"
)
...
st.write(
    "Upload a CSV file of sidewalk segment data to compute ADA, risk, and priority scores."
)
...
st.write(
    "Upload a sidewalk image to classify sidewalk conditions using AI."
)
        ...
        if uploaded_image is not None:
            ...
            if "top" in prediction:
                ...
                if isinstance(predictions, dict):
                    ...
                    predicted_class = top_prediction.get(
                        "class",
                        "Unknown"
                    )
            ...
            if predicted_class == "surface_problem":
                ...
                st.warning(
                    "Potential sidewalk surface issue detected."
                )

            elif predicted_class == "obstacle":

                st.warning(
                    "Potential sidewalk obstruction detected."
                )
                ...
                st.success(
                    "No major sidewalk issue detected."
                )
...
if uploaded_file is not None:
    ...
    try:
        ...
        col2.metric(
            "Avg ADA Score",
            f"{scored_df['ada_score'].mean():.1f}"
        )

        col3.metric(
            "Avg Risk Score",
            f"{scored_df['risk_score'].mean():.1f}"
        )

        col4.metric(
            "Avg Priority Score",
            f"{scored_df['priority_score'].mean():.1f}"
        )
